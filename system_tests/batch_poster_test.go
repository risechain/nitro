...
func testBatchPosterParallel(t *testing.T, useRedis bool) {
	...
	for i := 0; i < parallelBatchPosters; i++ {
		...
		batchPoster, err := arbnode.NewBatchPoster(nil, nodeA.L1Reader, nodeA.InboxTracker, nodeA.TxStreamer, nodeA.SyncMonitor, func() *arbnode.BatchPosterConfig { return &batchPosterConfig }, nodeA.DeployInfo, &seqTxOpts, nil, nil)
		Require(t, err)
